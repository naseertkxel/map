...
const VipOrderScreen = ({route}) => {
  ...
  const taxiTypesArabic = ['حجز لشخص واحد', 'حجز لمجموعة'];
  const taxiTypesEnglish = ['one', 'many'];
  ...
  console.log(groupPhoneNumbers);
  const [groupPhoneErrors, setGroupPhoneErrors] = useState({});
  ...
  const handleGroupPhoneChange = async (text, numberIndex) => {
    const newGroupPhoneNumbers = {...groupPhoneNumbers};
    newGroupPhoneNumbers[`number${numberIndex}`] = text;
    setGroupPhoneNumbers(newGroupPhoneNumbers);

    const newGroupPhoneErrors = {...groupPhoneErrors};

    if (text.length > 0) {
      try {
        const response = await fetch(
          `https://zyvlz2xt17.execute-api.us-east-1.amazonaws.com/dev/phone/users/${text}`,
        );
        const data = await response.json();
        if (data.exists) {
          delete newGroupPhoneErrors[`number${numberIndex}`];
        } else {
          newGroupPhoneErrors[`number${numberIndex}`] = 'رقم الهاتف غير موجود';
        }
      } catch (error) {
    ...
    setGroupPhoneErrors(newGroupPhoneErrors);
  };

  const renderGroupPhoneInput = numberIndex => (
    <View key={`groupPhoneInput-${numberIndex}`} style={{marginBottom: 0}}>
      <TextInput
        style={{
          borderTopWidth: 0,
          borderLeftWidth: 0,
        ...
        className="text-sm font-bold text-right w-[75%] mx-auto pl-3 pr-3"
        onChangeText={text => handleGroupPhoneChange(text, numberIndex)}
        value={groupPhoneNumbers[`number${numberIndex}`]}
        keyboardType="phone-pad"
        placeholder={`رقم موبايل المشترك ${numberIndex}`}
      />
      {groupPhoneErrors[`number${numberIndex}`] && (
  ...
  const isNextButtonDisabled = () => {
    const isValidGroupPhone =
      taxiType === 'many' &&
      Object.keys(groupPhoneNumbers).some(
        key => groupPhoneNumbers[key] && !groupPhoneErrors[key],
      ) &&
      !Object.keys(groupPhoneErrors).some(
        key => groupPhoneNumbers[key] && groupPhoneErrors[key],
      );

    return !(
      taxiType &&
      (taxiType === 'one' || isValidGroupPhone) &&
      typeOfWork &&
  ...
  const getFilledGroupPhoneNumbers = () => {
    return Object.keys(groupPhoneNumbers)
      .filter(key => groupPhoneNumbers[key])
      .reduce((obj, key) => {
        obj[key] = groupPhoneNumbers[key];
        return obj;
      }, {});
  };
  ...
  return (
    <SafeAreaView style={{backgroundColor: 'white', flex: 1}}>
      ...
      <Text className="mx-auto text-black text-2xl font-bold">الحجز الخاص</Text>
      ...
      {loading ? (
        ...
        <View className="flex mt-2 space-y-2 flex-1 h-screen">
          <View className="w-[75%] mx-auto">
            <SelectDropdown
              data={citiesData}
              onSelect={(selectedItem, index) => {
                handleBookingTypeChange(selectedItem.english);
                toggleCity(selectedItem, index);
              }}
              defaultButtonText={'نوع الحجز'}
          ...
          {bookingType === 'many' && (
            <>
              {numberOfInputs >= 1 && renderGroupPhoneInput(1)}
              {numberOfInputs >= 2 && renderGroupPhoneInput(2)}
              {numberOfInputs >= 3 && renderGroupPhoneInput(3)}
            </>
          )}
          {bookingType === 'many' && numberOfInputs < 3 && (
            <TouchableOpacity
              className="mx-auto w-36 h-10 rounded-full top-1 bottom-1"
              ...
              <LinearGradient
                ...
                <Text className="text-center my-auto text-white text-lg font-bold">
                  اضافة مشترك اخر
                </Text>
              </LinearGradient>
            </TouchableOpacity>
              ...
              onPress={() => {
                if (location) {
                  navigation.navigate('MapNew', {
                    ...
                    phoneNumber,
                    fromlocation: location,
                    groupPhoneNumbers: getFilledGroupPhoneNumbers(),
                    screentype: 'vip',
                  });
                } else {
                  console.error(
          ...
          <View style={{flex: 1, justifyContent: 'flex-end'}}>
            {showErrorMessage && (
              <Text
                style={{color: 'red', textAlign: 'center', marginBottom: 1}}>
...
export default React.memo(VipOrderScreen);
